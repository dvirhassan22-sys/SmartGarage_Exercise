...
namespace SmartGarage_Exercise
{
    ...
    public static class SmartGarageTester
    {
        ...
        /// </summary>
        public static void MoveAllRight(IMovable[] MovingThings)
        {
            foreach (IMovable movable in MovingThings)
            {
                movable.MoveRight();
            }
        }
              public static void MoveAllLeft(IMovable[] MovingThings)
        {
            foreach (IMovable movable in MovingThings)
            {
                movable.MoveLeft();
            } }
              public static void MoveAllBackward(IMovable[] MovingThings)
        {
            foreach (IMovable movable in MovingThings)
            {
                movable.MoveBackward();
            } }
        public static void MoveAllForward(IMovable[] MovingThings)
        {
            foreach (IMovable movable in MovingThings)
            {
                movable.MoveForward();
        ...
        public static void ChargeMoving(IMovable[] movable)
        {
            foreach(var item in movable)
            {
                if(item is IChargable)
                {
                    ((IChargable) item).ChargeBattery();
                }
            }
        }
